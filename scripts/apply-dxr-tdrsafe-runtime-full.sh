...
if contains src/opengl/gl_d3d12raylight.cpp "DXR TDRSAFE" && contains src/renderer/tr_init.cpp "r_dxrFreezeScene"; then
  echo "DXR TDR-safe runtime fix already detected; nothing to apply."
  exit 0
fi

if contains src/opengl/gl_d3d12raylight.cpp "DXR HALFRES" && contains src/renderer/tr_init.cpp "r_dxrRenderScale"; then
  echo "Existing HalfRes stack detected; applying only patch 09."
  ...
  echo "Existing Dispatch Guard stack detected; applying patches 08-09."
  patches=(
  ...
  echo "Existing Safe Mode stack detected; applying patches 07-09."
  patches=(
  ...
  echo "Base tree detected; applying full DXR stack 00-09."
  patches=(
...
echo "DXR TDR-safe runtime stack is ready."
