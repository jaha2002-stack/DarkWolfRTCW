...
if contains src/opengl/gl_d3d12raylight.cpp "DXR FINAL HQ SMOOTH" && contains src/renderer/tr_init.cpp 'r_dxrDispatchWidth", "160"'; then
  echo "DXR Final HighQuality Stable patch already detected; nothing to apply."
  exit 0
fi
...
if contains src/opengl/gl_d3d12raylight.cpp "Playable TDR-safe composite" && contains src/renderer/tr_init.cpp 'r_dxrLegacyBlend", "0.90"'; then
  echo "Existing Playable TDR-safe stack detected; applying only patch 11."
  ...
  echo "Existing TDR-safe runtime stack detected; applying patches 10-11."
  patches=(
  ...
  echo "Existing HalfRes stack detected; applying patches 09-11."
  patches=(
  ...
  echo "Existing Dispatch Guard stack detected; applying patches 08-11."
  patches=(
  ...
  echo "Existing Safe Mode stack detected; applying patches 07-11."
  patches=(
  ...
  echo "Base tree detected; applying full DXR final high-quality stack 00-11."
  patches=(
...
echo "DXR Final HighQuality Stable stack is ready."
