...
try {
    ...
    if ((Test-Contains 'src/opengl/gl_d3d12raylight.cpp' 'DXR TDRSAFE') -and (Test-Contains 'src/renderer/tr_init.cpp' 'r_dxrFreezeScene')) {
        Write-Host 'DXR TDR-safe runtime fix already detected; nothing to apply.'
        return
    }

    if ((Test-Contains 'src/opengl/gl_d3d12raylight.cpp' 'DXR HALFRES') -and (Test-Contains 'src/renderer/tr_init.cpp' 'r_dxrRenderScale')) {
        Write-Host 'Existing HalfRes stack detected; applying only patch 09.'
        $patches = @('patches/09-dxr-tdr-safe-runtime-freeze.patch')
    }
    elseif ((Test-Contains 'src/opengl/gl_d3d12raylight.cpp' 'gRaygenMode') -and (Test-Contains 'src/renderer/tr_init.cpp' 'r_dxrDispatchMode')) {
        Write-Host 'Existing Dispatch Guard stack detected; applying patches 08-09.'
        $patches = @(
    ...
    elseif ((Test-Contains 'src/opengl/gl_d3d12raylight.cpp' 'glRaytracingSetSafetyOptions') -and (Test-Contains 'src/renderer/tr_init.cpp' 'r_dxrSafeMode')) {
        Write-Host 'Existing Safe Mode stack detected; applying patches 07-09.'
        $patches = @(
        ...
        Write-Host 'Base tree detected; applying full DXR stack 00-09.'
        $patches = @(
    ...
    Write-Host 'DXR TDR-safe runtime stack is ready.'
}
finally {
    Pop-Location
}
