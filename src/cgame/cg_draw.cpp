...
void CG_DrawHead( float x, float y, float w, float h, int clientNum, vec3_t headAngles ) {
	...
	if ( cg_draw3dIcons.integer ) {
		...
		sys->R_ModelBounds( cm, mins, maxs );
...
*/
			...
			if ( ps->grenadeTimeLeft ) {
				if ( ps->weapon == WP_DYNAMITE ) {
					...
					if ( ( ( cg.grenLastTime ) % 1000 ) < ( ( ps->grenadeTimeLeft ) % 1000 ) ) {
						...
						case 3:
							sys->S_StartLocalSound( cgs.media.grenadePulseSound4, CHAN_LOCAL_SOUND );
							break;
						case 2:
							sys->S_StartLocalSound( cgs.media.grenadePulseSound3, CHAN_LOCAL_SOUND );
							break;
						case 1:
							sys->S_StartLocalSound( cgs.media.grenadePulseSound2, CHAN_LOCAL_SOUND );
							break;
						case 0:
							sys->S_StartLocalSound( cgs.media.grenadePulseSound1, CHAN_LOCAL_SOUND );
							break;
						}
					}
				}
	...
	if ( value > 100 ) {
		sys->R_SetColor( colors[3] );       // white
	} else if ( value > 25 ) {
		sys->R_SetColor( colors[0] );   // green
	} else if ( value > 0 ) {
		color = ( cg.time >> 8 ) & 1; // flash
		sys->R_SetColor( colors[color] );
	} else {
		sys->R_SetColor( colors[1] );   // red
	}
			...
			for ( j = 0; j < PW_NUM_POWERUPS; j++ ) {
				if ( ci->powerups & ( 1 << j ) ) {
					...
					if ( item != NULL ) { // JPW NERVE added for invulnerability powerup at beginning of map
						CG_DrawPic( xx, y, TINYCHAR_WIDTH, TINYCHAR_HEIGHT,
									sys->R_RegisterShader( item->icon ) );
						xx -= TINYCHAR_WIDTH;
					} // jpw
				}
			}
...
static float CG_DrawPowerups( float y ) {
	...
	for ( i = 0 ; i < active ; i++ ) {
		...
		CG_DrawPic( 640 - size, y + ICON_SIZE / 2 - size / 2,
					size, size, sys->R_RegisterShader( item->icon ) );
	}
	sys->R_SetColor( NULL );
...
static void CG_DrawDisconnect( void ) {
	...
	cmdNum = sys->GetCurrentCmdNumber() - CMD_BACKUP + 1;
	sys->GetUserCmd( cmdNum, &cmd );
	...
	CG_DrawPic( x, y, 48, 48, sys->R_RegisterShader( "gfx/2d/net.tga" ) );
}
...
//	if(0) {
	...
	for ( a = 0 ; a < aw ; a++ ) {
		...
		if ( v > 0 ) {
			if ( color != 1 ) {
				color = 1;
				sys->R_SetColor( g_color_table[ColorIndex( COLOR_YELLOW )] );
			}
			if ( v > range ) {
				v = range;
			}
			sys->R_DrawStretchPic( ax + aw - a, mid - v, 1, v, 0, 0, 0, 0, cgs.media.whiteShader );
		} else if ( v < 0 ) {
			if ( color != 2 ) {
				color = 2;
				sys->R_SetColor( g_color_table[ColorIndex( COLOR_BLUE )] );
			}
			v = -v;
			if ( v > range ) {
				v = range;
			}
			sys->R_DrawStretchPic( ax + aw - a, mid, 1, v, 0, 0, 0, 0, cgs.media.whiteShader );
		}
	}
	...
	for ( a = 0 ; a < aw ; a++ ) {
		...
		if ( v > 0 ) {
			if ( lagometer.snapshotFlags[i] & SNAPFLAG_RATE_DELAYED ) {
				...
				if ( color != 3 ) {
					color = 3;
					sys->R_SetColor( g_color_table[ColorIndex( COLOR_GREEN )] );
				}
			}
			v = v * vscale;
			if ( v > range ) {
			...
			sys->R_DrawStretchPic( ax + aw - a, ay + ah - v, 1, v, 0, 0, 0, 0, cgs.media.whiteShader );
		} else if ( v < 0 ) {
			if ( color != 4 ) {
				color = 4;      // RED for dropped snapshots
				sys->R_SetColor( g_color_table[ColorIndex( COLOR_RED )] );
			}
			sys->R_DrawStretchPic( ax + aw - a, ay + ah - range, 1, range, 0, 0, 0, 0, cgs.media.whiteShader );
		}
	}
...
static void CG_DrawWeapReticle( void ) {
	...
	if ( weap == WP_SNIPERRIFLE ) {
		...
		if ( cgs.media.reticleShaderSimpleQ ) {
			sys->R_DrawStretchPic( x, 0, w, h, 0, 0, 1, 1, cgs.media.reticleShaderSimpleQ );    // tl
			sys->R_DrawStretchPic( x + w, 0, w, h, 1, 0, 0, 1, cgs.media.reticleShaderSimpleQ );  // tr
			sys->R_DrawStretchPic( x, h, w, h, 0, 1, 1, 0, cgs.media.reticleShaderSimpleQ );    // bl
			sys->R_DrawStretchPic( x + w, h, w, h, 1, 1, 0, 0, cgs.media.reticleShaderSimpleQ );  // br
		}
...
static void CG_DrawBinocReticle( void ) {
	...
	if ( cgs.media.binocShaderSimpleQ ) {
		CG_AdjustFrom640( &x, &y, &w, &h );
		sys->R_DrawStretchPic( 0, 0, w, h, 0, 0, 1, 1, cgs.media.binocShaderSimpleQ );  // tl
		sys->R_DrawStretchPic( w, 0, w, h, 1, 0, 0, 1, cgs.media.binocShaderSimpleQ );  // tr
		sys->R_DrawStretchPic( 0, h, w, h, 0, 1, 1, 0, cgs.media.binocShaderSimpleQ );  // bl
		sys->R_DrawStretchPic( w, h, w, h, 1, 1, 0, 0, cgs.media.binocShaderSimpleQ );  // br
	}
...
//----(SA)	end
	...
	sys->R_DrawStretchPic(  x + cg.refdef.x + 0.5 * ( cg.refdef.width - w ),
...
//	traceEnt = &g_entities[trace.entityNum];
	...
	content = sys->CM_PointContents( trace.endpos, 0 );
...
static void CG_DrawVote( void ) {
	...
	if ( cgs.voteModified ) {
		...
		sys->S_StartLocalSound( cgs.media.talkSound, CHAN_LOCAL_SOUND );
	}
...
static void CG_ActivateLimboMenu( void ) {
	...
	if ( test && !latch ) {
		sys->SendConsoleCommand( "startLimboMode\n" );
		sys->SendConsoleCommand( "OpenLimboMenu\n" );
		latch = qtrue;
	}
	else if ( !test && latch ) {
		sys->SendConsoleCommand( "stopLimboMode\n" );
		sys->SendConsoleCommand( "CloseLimboMenu\n" );
		latch = qfalse;
	}

	// set the limbo state
	sys->Cvar_VariableStringBuffer( "ui_limboMode", buf, sizeof( buf ) );
...
static void CG_DrawWarmup( void ) {
	...
	if ( sec != cg.warmupCount ) {
		...
		switch ( sec ) {
		case 0:
			sys->S_StartLocalSound( cgs.media.count1Sound, CHAN_ANNOUNCER );
			break;
		case 1:
			sys->S_StartLocalSound( cgs.media.count2Sound, CHAN_ANNOUNCER );
			break;
		case 2:
			sys->S_StartLocalSound( cgs.media.count3Sound, CHAN_ANNOUNCER );
			break;
		default:
			break;
		}
...
static void CG_DrawFlashFade( void ) {
	...
	if ( cgs.scrFadeStartTime + cgs.scrFadeDuration < cg.time ) {
		...
		elapsed = ( time = sys->Milliseconds() ) - lastTime;  // we need to use sys->Milliseconds() here since the cg.time gets modified upon reloading
		lastTime = time;
...
static void CG_DrawFlashFire( void ) {
	...
	if ( alpha > 0 ) {
		...
		sys->S_AddLoopingSound( cg.snap->ps.clientNum, cg.snap->ps.origin, vec3_origin, cgs.media.flameSound, (int)( 255.0 * alpha ) );
		sys->S_AddLoopingSound( cg.snap->ps.clientNum, cg.snap->ps.origin, vec3_origin, cgs.media.flameCrackSound, (int)( 255.0 * alpha ) );
	} else {
		cg.v_noFireTime = cg.time;
	}
}
...
void CG_DrawActive( stereoFrame_t stereoView ) {
	...
	if ( cg_gameType.integer == GT_SINGLE_PLAYER ) {
		if ( strlen( cg_missionStats.string ) > 1 ) {
			sys->Cvar_Set( "com_expectedhunkusage", "-2" );
			CG_DrawInformation();
			return;
		}
	}
