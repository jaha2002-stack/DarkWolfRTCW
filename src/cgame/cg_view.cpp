...
void CG_TestModel_f( void ) {
	...
	cg.testModelEntity.hModel = sys->R_RegisterModel( cg.testModelName );
...
static void CG_AddTestModel( void ) {
	...
	sys->R_AddRefEntityToScene( &cg.testModelEntity );
}
...
static void CG_CalcVrect( void ) {
	...
	if ( cg.snap->ps.pm_type == PM_INTERMISSION ) {
		...
		if ( cg_viewsize.integer < 30 ) {
			sys->Cvar_Set( "cg_viewsize","30" );
			xsize = ysize = 30;
		} else if ( cg_viewsize.integer > 100 ) {
			sys->Cvar_Set( "cg_viewsize","100" );
			xsize = ysize = 100;
		} else {
...
//	sys->Cvar_Set( "cg_recoilPitch", va("%f", cg.recoilPitchAngle) );
}
...
//	sys->S_AddLoopingSound( cent->currentState.number, cent->lerpOrigin, vec3_origin, cgs.media.underWaterSound, 255 );
	sys->S_AddLoopingSound( cg.snap->ps.clientNum, cg.snap->ps.origin, vec3_origin, cgs.media.underWaterSound, 255 & ( 1 << 8 ) );
}
...
static int CG_CalcViewValues( void ) {
	...
	if ( cg.cameraMode ) {
		...
		if ( sys->GetCameraInfo( CAM_PRIMARY, cg.time, &origin, &angles, &fov ) ) {
			...
			sys->SendClientCommand( va( "setCameraOrigin %f %f %f", origin[0], origin[1], origin[2] ) );
			return 0;
...
static void CG_PowerupTimerSounds( void ) {
	...
	for ( i = 0 ; i < MAX_POWERUPS ; i++ ) {
		...
		if ( ( t - cg.time ) / POWERUP_BLINK_TIME != ( t - cg.oldTime ) / POWERUP_BLINK_TIME ) {
			sys->S_StartSound( NULL, cg.snap->ps.clientNum, CHAN_ITEM, cgs.media.wearOffSound );
		}
	}
}
		...
		if ( !token || !token[0] ) {
			...
			if ( atoi( token ) ) {   // this camera has fog
				...
				if ( 1 ) {
					...
					sys->R_SetFog( FOG_PORTALVIEW, fogStart, fogEnd, fogColor[0], fogColor[1], fogColor[2], 1.1 );
					foginited = qtrue;
				}
			} else {
				if ( !foginited ) {
					sys->R_SetFog( FOG_PORTALVIEW, 0,0,0,0,0,0 ); // init to null
					foginited = qtrue;
				}
			}
		}
...
#define DEBUGTIME CG_Printf( "t%i:%i ", dbgCnt++, elapsed = ( sys->Milliseconds() - dbgTime ) ); dbgTime += elapsed;
#else
#define DEBUGTIME
#endif
...
#ifdef DEBUGTIME_ENABLED
	int dbgTime = sys->Milliseconds(),elapsed;
	int dbgCnt = 0;
#endif
...
*/
	...
	sys->S_ClearLoopingSounds( qfalse );
	...
	sys->SetUserCmdValue( cg.weaponSelect, cg.holdableSelect, cg.zoomSensitivity, cg.cld );
	...
	sys->S_Respatialize( cg.snap->ps.clientNum, cg.refdef.vieworg, cg.refdef.viewaxis, inwater );
