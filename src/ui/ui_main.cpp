...
void AssetCache() {
	...
	uiInfo.uiDC.Assets.gradientBar = sys->R_RegisterShaderNoMip( ASSET_GRADIENTBAR );
	uiInfo.uiDC.Assets.fxBasePic = sys->R_RegisterShaderNoMip( ART_FX_BASE );
	uiInfo.uiDC.Assets.fxPic[0] = sys->R_RegisterShaderNoMip( ART_FX_RED );
	uiInfo.uiDC.Assets.fxPic[1] = sys->R_RegisterShaderNoMip( ART_FX_YELLOW );
	uiInfo.uiDC.Assets.fxPic[2] = sys->R_RegisterShaderNoMip( ART_FX_GREEN );
	uiInfo.uiDC.Assets.fxPic[3] = sys->R_RegisterShaderNoMip( ART_FX_TEAL );
	uiInfo.uiDC.Assets.fxPic[4] = sys->R_RegisterShaderNoMip( ART_FX_BLUE );
	uiInfo.uiDC.Assets.fxPic[5] = sys->R_RegisterShaderNoMip( ART_FX_CYAN );
	uiInfo.uiDC.Assets.fxPic[6] = sys->R_RegisterShaderNoMip( ART_FX_WHITE );
	uiInfo.uiDC.Assets.scrollBar = sys->R_RegisterShaderNoMip( ASSET_SCROLLBAR );
	uiInfo.uiDC.Assets.scrollBarArrowDown = sys->R_RegisterShaderNoMip( ASSET_SCROLLBAR_ARROWDOWN );
	uiInfo.uiDC.Assets.scrollBarArrowUp = sys->R_RegisterShaderNoMip( ASSET_SCROLLBAR_ARROWUP );
	uiInfo.uiDC.Assets.scrollBarArrowLeft = sys->R_RegisterShaderNoMip( ASSET_SCROLLBAR_ARROWLEFT );
	uiInfo.uiDC.Assets.scrollBarArrowRight = sys->R_RegisterShaderNoMip( ASSET_SCROLLBAR_ARROWRIGHT );
	uiInfo.uiDC.Assets.scrollBarThumb = sys->R_RegisterShaderNoMip( ASSET_SCROLL_THUMB );
	uiInfo.uiDC.Assets.sliderBar = sys->R_RegisterShaderNoMip( ASSET_SLIDER_BAR );
	uiInfo.uiDC.Assets.sliderThumb = sys->R_RegisterShaderNoMip( ASSET_SLIDER_THUMB );

	for ( n = 0; n < NUM_CROSSHAIRS; n++ ) {
		uiInfo.uiDC.Assets.crosshairShader[n] = sys->R_RegisterShaderNoMip( va( "gfx/2d/crosshair%c", 'a' + n ) );
	}

//	uiInfo.newHighScoreSound = sys->S_RegisterSound("sound/feedback/voc_newhighscore.wav");
}

void _UI_DrawSides( float x, float y, float w, float h, float size ) {
	UI_AdjustFrom640( &x, &y, &w, &h );
	size *= uiInfo.uiDC.xscale;
	sys->R_DrawStretchPic( x, y, size, h, 0, 0, 0, 0, uiInfo.uiDC.whiteShader );
	sys->R_DrawStretchPic( x + w - size, y, size, h, 0, 0, 0, 0, uiInfo.uiDC.whiteShader );
}

void _UI_DrawTopBottom( float x, float y, float w, float h, float size ) {
	UI_AdjustFrom640( &x, &y, &w, &h );
	size *= uiInfo.uiDC.yscale;
	sys->R_DrawStretchPic( x, y, w, size, 0, 0, 0, 0, uiInfo.uiDC.whiteShader );
	sys->R_DrawStretchPic( x, y + h - size, w, size, 0, 0, 0, 0, uiInfo.uiDC.whiteShader );
}
/*
================
UI_DrawRect
...
void Text_PaintChar( float x, float y, float width, float height, int font, float scale, float s, float t, float s2, float t2, qhandle_t hShader ) {
	...
	h = height * scale;
	UI_AdjustFrom640( &x, &y, &w, &h );
	sys->R_DrawStretchPic( x, y, w, h, s, t, s2, t2, hShader );
}
...
void UI_ShowPostGame( qboolean newHigh ) {
	sys->Cvar_Set( "cg_cameraOrbit", "0" );
	sys->Cvar_Set( "cg_thirdPerson", "0" );
	sys->Cvar_Set( "sv_killserver", "1" );
...
void _UI_Refresh( int realtime ) {
	...
	//if ( !( sys->Key_GetCatcher() & KEYCATCH_UI ) ) {
	//	return;
	//}
...
qboolean Asset_Parse( int handle ) {
	...
	while ( 1 ) {
		...
		if ( Q_stricmp( token.string, "font" ) == 0 ) {
			...
			sys->R_RegisterFont( tempStr, pointSize, &uiInfo.uiDC.Assets.textFont );
		...
		if ( Q_stricmp( token.string, "smallFont" ) == 0 ) {
			...
			sys->R_RegisterFont( tempStr, pointSize, &uiInfo.uiDC.Assets.smallFont );
			continue;
		}
		...
		if ( Q_stricmp( token.string, "bigFont" ) == 0 ) {
			...
			sys->R_RegisterFont( tempStr, pointSize, &uiInfo.uiDC.Assets.bigFont );
			continue;
		}
		...
		if ( Q_stricmp( token.string, "handwritingFont" ) == 0 ) {
			...
			sys->R_RegisterFont( tempStr, pointSize, &uiInfo.uiDC.Assets.handwritingFont );
			continue;
		}
		...
		if ( Q_stricmp( token.string, "gradientbar" ) == 0 ) {
			...
			uiInfo.uiDC.Assets.gradientBar = sys->R_RegisterShaderNoMip( tempStr );
			continue;
		}
		...
		if ( Q_stricmp( token.string, "menuEnterSound" ) == 0 ) {
			...
			uiInfo.uiDC.Assets.menuEnterSound = sys->S_RegisterSound( tempStr );
			continue;
		}
		...
		if ( Q_stricmp( token.string, "menuExitSound" ) == 0 ) {
			...
			uiInfo.uiDC.Assets.menuExitSound = sys->S_RegisterSound( tempStr );
			continue;
		}
		...
		if ( Q_stricmp( token.string, "itemFocusSound" ) == 0 ) {
			...
			uiInfo.uiDC.Assets.itemFocusSound = sys->S_RegisterSound( tempStr );
			continue;
		}
		...
		if ( Q_stricmp( token.string, "menuBuzzSound" ) == 0 ) {
			...
			uiInfo.uiDC.Assets.menuBuzzSound = sys->S_RegisterSound( tempStr );
			continue;
		}
		...
		if ( Q_stricmp( token.string, "cursor" ) == 0 ) {
			...
			uiInfo.uiDC.Assets.cursor = sys->R_RegisterShaderNoMip( uiInfo.uiDC.Assets.cursorStr );
			continue;
		}
...
#ifdef WOLF_SP_DEMO
		handle = sys->PC_LoadSource( "ui/menus.txt" );
#else
		handle = sys->PC_LoadSource( "ui/demomenus.txt" );
#endif
		if ( !handle ) {
			sys->Error( va( S_COLOR_RED "default menu file not found: ui/menus.txt, unable to continue!\n", menuFile ) );
		}
	}
...
static void UI_DrawHandicap( rectDef_t *rect, int font, float scale, vec4_t color, int textStyle ) {
	...
	h = Com_Clamp( 5, 100, sys->Cvar_VariableValue( "handicap" ) );
	i = 20 - h / 5;
...
static void UI_SetCapFragLimits( qboolean uiVars ) {
	...
	if ( uiVars ) {
		sys->Cvar_Set( "ui_captureLimit", va( "%d", cap ) );
		sys->Cvar_Set( "ui_fragLimit", va( "%d", frag ) );
	} else {
		sys->Cvar_Set( "capturelimit", va( "%d", cap ) );
		sys->Cvar_Set( "fraglimit", va( "%d", frag ) );
	}
}
...
static void UI_DrawNetGameType( rectDef_t *rect, int font, float scale, vec4_t color, int textStyle ) {
	if ( ui_netGameType.integer < 0 || ui_netGameType.integer > uiInfo.numGameTypes ) {
		sys->Cvar_Set( "ui_netGameType", "0" );
		sys->Cvar_Set( "ui_actualNetGameType", "0" );
	}
...
static void UI_DrawJoinGameType( rectDef_t *rect, int font, float scale, vec4_t color, int textStyle ) {
	if ( ui_joinGameType.integer < 0 || ui_joinGameType.integer > uiInfo.numJoinGameTypes ) {
		sys->Cvar_Set( "ui_joinGameType", "0" );
	}
...
static void UI_DrawSaveGameShot( rectDef_t *rect, float scale, vec4_t color ) {
	...
	if ( !strlen( ui_savegameName.string ) || ui_savegameName.string[0] == '0' ) {
		image = sys->R_RegisterShaderNoMip( "menu/art/unknownmap" );
	} else {
		i = UI_SavegameIndexFromName2( ui_savegameName.string );
//	mapName
		if ( uiInfo.savegameList[i].sshotImage == -1 ) {
			uiInfo.savegameList[i].sshotImage = sys->R_RegisterShaderNoMip( uiInfo.savegameList[i].imageName );
		}
...
static void UI_DrawClanLogo( rectDef_t *rect, float scale, vec4_t color ) {
	...
	if ( i >= 0 && i < uiInfo.teamCount ) {
		...
		if ( uiInfo.teamList[i].teamIcon == -1 ) {
			uiInfo.teamList[i].teamIcon         = sys->R_RegisterShaderNoMip( uiInfo.teamList[i].imageName );
			uiInfo.teamList[i].teamIcon_Metal   = sys->R_RegisterShaderNoMip( va( "%s_metal",uiInfo.teamList[i].imageName ) );
			uiInfo.teamList[i].teamIcon_Name    = sys->R_RegisterShaderNoMip( va( "%s_name", uiInfo.teamList[i].imageName ) );
		}
	...
	if ( i >= 0 && i < uiInfo.teamCount ) {
		...
		if ( uiInfo.teamList[i].cinematic >= -2 ) {
			if ( uiInfo.teamList[i].cinematic == -1 ) {
				uiInfo.teamList[i].cinematic = sys->CIN_PlayCinematic( va( "%s.roq", uiInfo.teamList[i].imageName ), 0, 0, 0, 0, ( CIN_loop | CIN_silent ) );
			}
			if ( uiInfo.teamList[i].cinematic >= 0 ) {
				sys->CIN_RunCinematic( uiInfo.teamList[i].cinematic );
				sys->CIN_SetExtents( uiInfo.teamList[i].cinematic, rect->x, rect->y, rect->w, rect->h );
				sys->CIN_DrawCinematic( uiInfo.teamList[i].cinematic );
			} else {
...
static void UI_DrawPregameCinematic( rectDef_t *rect, float scale, vec4_t color ) {
	if ( uiInfo.previewMovie > -2 ) {
		uiInfo.previewMovie = sys->CIN_PlayCinematic( va( "%s.roq", "assault" ), 0, 0, 0, 0, ( CIN_loop | CIN_silent | CIN_system ) );
		if ( uiInfo.previewMovie >= 0 ) {
			sys->CIN_RunCinematic( uiInfo.previewMovie );
			sys->CIN_SetExtents( uiInfo.previewMovie, rect->x, rect->y, rect->w, rect->h );
			sys->CIN_DrawCinematic( uiInfo.previewMovie );
		} else {
...
static void UI_DrawPreviewCinematic( rectDef_t *rect, float scale, vec4_t color ) {
	if ( uiInfo.previewMovie > -2 ) {
		uiInfo.previewMovie = sys->CIN_PlayCinematic( va( "%s.roq", uiInfo.movieList[uiInfo.movieIndex] ), 0, 0, 0, 0, ( CIN_loop | CIN_silent ) );
		if ( uiInfo.previewMovie >= 0 ) {
			sys->CIN_RunCinematic( uiInfo.previewMovie );
			sys->CIN_SetExtents( uiInfo.previewMovie, rect->x, rect->y, rect->w, rect->h );
			sys->CIN_DrawCinematic( uiInfo.previewMovie );
...
static void UI_DrawSkill( rectDef_t *rect, int font, float scale, vec4_t color, int textStyle ) {
	int i;
	i = sys->Cvar_VariableValue( "g_spSkill" );
...
static void UI_DrawTeamMember( rectDef_t *rect, int font, float scale, vec4_t color, qboolean blue, int num, int textStyle ) {
	...
	// 1 - Human
	// 2..NumCharacters - Bot
	int value = sys->Cvar_VariableValue( va( blue ? "ui_blueteam%i" : "ui_redteam%i", num ) );
	const char *text;
	if ( value <= 0 ) {
		text = "Closed";
	} else if ( value == 1 ) {
...
static void UI_DrawMapLevelshot( rectDef_t *rect ) {
	...
	if ( levelname && levelname[0] != 0 ) {
		levelshot = sys->R_RegisterShaderNoMip( va( "levelshots/%s.tga", levelname ) );
	}

	if ( !levelshot ) {
		levelshot = sys->R_RegisterShaderNoMip( "menu/art/unknownmap" );
	}
...
//		flags|=1;	// BAR_LEFT (left, when vertical means grow 'up')
	...
	sys->Cvar_VariableStringBuffer( "com_expectedhunkusage", hunkBuf, MAX_QPATH );
...
static void UI_DrawMapPreview( rectDef_t *rect, float scale, vec4_t color, qboolean net ) {
	...
	if ( map < 0 || map > uiInfo.mapCount ) {
		if ( net ) {
			...
			sys->Cvar_Set( "ui_currentNetMap", "0" );
		} else {
			ui_currentMap.integer = 0;
			sys->Cvar_Set( "ui_currentMap", "0" );
		}
		map = 0;
	}

	if ( uiInfo.mapList[map].levelShot == -1 ) {
		uiInfo.mapList[map].levelShot = sys->R_RegisterShaderNoMip( uiInfo.mapList[map].imageName );
	}

	if ( uiInfo.mapList[map].levelShot > 0 ) {
		UI_DrawHandlePic( rect->x, rect->y, rect->w, rect->h, uiInfo.mapList[map].levelShot );
	} else {
		UI_DrawHandlePic( rect->x, rect->y, rect->w, rect->h, sys->R_RegisterShaderNoMip( "menu/art/unknownmap" ) );
	}
}
...
static void UI_DrawMapCinematic( rectDef_t *rect, float scale, vec4_t color, qboolean net ) {
	...
	if ( uiInfo.mapList[map].cinematic >= -1 ) {
		if ( uiInfo.mapList[map].cinematic == -1 ) {
			uiInfo.mapList[map].cinematic = sys->CIN_PlayCinematic( va( "%s.roq", uiInfo.mapList[map].mapLoadName ), 0, 0, 0, 0, ( CIN_loop | CIN_silent ) );
		}
		if ( uiInfo.mapList[map].cinematic >= 0 ) {
			sys->CIN_RunCinematic( uiInfo.mapList[map].cinematic );
			sys->CIN_SetExtents( uiInfo.mapList[map].cinematic, rect->x, rect->y, rect->w, rect->h );
			sys->CIN_DrawCinematic( uiInfo.mapList[map].cinematic );
		} else {
...
static void UI_DrawPlayerModel( rectDef_t *rect ) {
	...
	if ( sys->Cvar_VariableValue( "ui_Q3Model" ) ) {
	...
	// compare new cvars to old cvars and see if we need to update
	{
		...
		v1 = sys->Cvar_VariableValue( "mp_team" );
		v2 = sys->Cvar_VariableValue( "ui_prevTeam" );
		if ( v1 != v2 ) {
			sys->Cvar_Set( "ui_prevTeam", va( "%i", v1 ) );
			updateModel = qtrue;
		}

		v1 = sys->Cvar_VariableValue( "mp_playerType" );
		v2 = sys->Cvar_VariableValue( "ui_prevClass" );
		if ( v1 != v2 ) {
			sys->Cvar_Set( "ui_prevClass", va( "%i", v1 ) );
			updateModel = qtrue;
		}

		v1 = sys->Cvar_VariableValue( "mp_weapon" );
		v2 = sys->Cvar_VariableValue( "ui_prevWeapon" );
		if ( v1 != v2 ) {
			sys->Cvar_Set( "ui_prevWeapon", va( "%i", v1 ) );
			updateModel = qtrue;
		}
	}
...
static void UI_DrawNetMapCinematic( rectDef_t *rect, float scale, vec4_t color ) {
	...
	if ( uiInfo.serverStatus.currentServerCinematic >= 0 ) {
		sys->CIN_RunCinematic( uiInfo.serverStatus.currentServerCinematic );
		sys->CIN_SetExtents( uiInfo.serverStatus.currentServerCinematic, rect->x, rect->y, rect->w, rect->h );
		sys->CIN_DrawCinematic( uiInfo.serverStatus.currentServerCinematic );
	} else {
...
static void UI_DrawTier( rectDef_t *rect, int font, float scale, vec4_t color, int textStyle ) {
	int i;
	i = sys->Cvar_VariableValue( "ui_currentTier" );
...
static void UI_DrawTierMap( rectDef_t *rect, int index ) {
	...
	if ( uiInfo.tierList[i].mapHandles[index] == -1 ) {
		uiInfo.tierList[i].mapHandles[index] = sys->R_RegisterShaderNoMip( va( "levelshots/%s", uiInfo.tierList[i].maps[index] ) );
	}
...
static void UI_DrawTierMapName( rectDef_t *rect, int font, float scale, vec4_t color, int textStyle ) {
	...
	j = sys->Cvar_VariableValue( "ui_currentMap" );
...
static void UI_NextOpponent() {
	...
	sys->Cvar_Set( "ui_opponentName", uiInfo.teamList[i].teamName );
}
...
//----(SA)	end
	...
	case UI_BLUETEAM4:
	case UI_BLUETEAM5:
		value = sys->Cvar_VariableValue( va( "ui_blueteam%i", ownerDraw - UI_BLUETEAM1 + 1 ) );
		if ( value <= 0 ) {
			text = "Closed";
		} else if ( value == 1 ) {
			text = "Human";
	...
	case UI_REDTEAM4:
	case UI_REDTEAM5:
		value = sys->Cvar_VariableValue( va( "ui_redteam%i", ownerDraw - UI_REDTEAM1 + 1 ) );
		if ( value <= 0 ) {
			text = "Closed";
		} else if ( value == 1 ) {
			text = "Human";
...
static void UI_DrawBotName( rectDef_t *rect, int font, float scale, vec4_t color, int textStyle ) {
	...
	int game = sys->Cvar_VariableValue( "g_gametype" );
	const char *text = "";
	if ( game >= GT_TEAM ) {
		if ( value >= uiInfo.characterCount ) {
			value = 0;
...
static void UI_BuildPlayerList() {
	...
	sys->GetClientState( &cs );
	sys->GetConfigString( CS_PLAYERS + cs.clientNum, info, MAX_INFO_STRING );
	...
	playerTeamNumber = 0;
	for ( n = 0; n < count; n++ ) {
		sys->GetConfigString( CS_PLAYERS + n, info, MAX_INFO_STRING );
	...
	if ( !uiInfo.teamLeader ) {
		sys->Cvar_Set( "cg_selectedPlayer", va( "%d", playerTeamNumber ) );
	}

	n = sys->Cvar_VariableValue( "cg_selectedPlayer" );
	if ( n < 0 || n > uiInfo.myTeamCount ) {
		n = 0;
	}
	if ( n < uiInfo.myTeamCount ) {
		sys->Cvar_Set( "cg_selectedPlayerName", uiInfo.teamNames[n] );
	}
}
...
static void UI_DrawServerRefreshDate( rectDef_t *rect, int font, float scale, vec4_t color, int textStyle ) {
	...
	if ( uiInfo.serverStatus.refreshActive ) {
		...
		Text_Paint( rect->x, rect->y, font, scale, newColor, va( "Getting info for %d servers (ESC to cancel)", sys->LAN_GetServerCount( ui_netSource.integer ) ), 0, 0, textStyle );
	} else {
		char buff[64];
...
static qboolean UI_OwnerDrawVisible( int flags ) {
	...
	while ( flags ) {
		...
		if ( flags & UI_SHOW_FFA ) {
			if ( sys->Cvar_VariableValue( "g_gametype" ) != GT_FFA ) {
				vis = qfalse;
			}
			flags &= ~UI_SHOW_FFA;
		}
		...
		if ( flags & UI_SHOW_NEWHIGHSCORE ) {
			if ( uiInfo.newHighScoreTime < uiInfo.uiDC.realTime ) {
				...
				if ( uiInfo.soundHighScore ) {
					if ( sys->Cvar_VariableValue( "sv_killserver" ) == 0 ) {
						// wait on server to go down before playing sound
						sys->S_StartLocalSound( uiInfo.newHighScoreSound, CHAN_ANNOUNCER );
...
static qboolean UI_Effects_HandleKey( int flags, float *special, int key ) {
	if ( key == K_MOUSE1 || key == K_MOUSE2 || key == K_ENTER || key == K_KP_ENTER ) {
		...
		sys->Cvar_SetValue( "color", uitogamecode[uiInfo.effectsColor] );
		return qtrue;
	}
	return qfalse;
}
...
static qboolean UI_SavegameName_HandleKey( int flags, float *special, int key ) {
	...
	if ( key == K_MOUSE1 || key == K_MOUSE2 || key == K_ENTER || key == K_KP_ENTER ) {
		...
		sys->Cvar_Set( "ui_savegameName", uiInfo.savegameList[uiInfo.savegameStatus.displaySavegames[i]].savegameName );
		sys->Cvar_Set( "ui_savegameInfo", uiInfo.savegameList[uiInfo.savegameStatus.displaySavegames[i]].savegameInfoText );
		return qtrue;
	}
	return qfalse;
}
...
static qboolean UI_ClanName_HandleKey( int flags, float *special, int key ) {
	if ( key == K_MOUSE1 || key == K_MOUSE2 || key == K_ENTER || key == K_KP_ENTER ) {
		...
		i = UI_TeamIndexFromName( UI_Cvar_VariableString( "ui_teamName" ) );
		if ( uiInfo.teamList[i].cinematic >= 0 ) {
			sys->CIN_StopCinematic( uiInfo.teamList[i].cinematic );
		...
		sys->Cvar_Set( "ui_teamName", uiInfo.teamList[i].teamName );
		updateModel = qtrue;
		return qtrue;
	}
	return qfalse;
...
static qboolean UI_GameType_HandleKey( int flags, float *special, int key, qboolean resetMap ) {
	...
	if ( key == K_MOUSE1 || key == K_MOUSE2 || key == K_ENTER || key == K_KP_ENTER ) {
		...
		if ( uiInfo.gameTypes[ui_gameType.integer].gtEnum == GT_TOURNAMENT ) {
			sys->Cvar_Set( "ui_Q3Model", "1" );
		} else {
			sys->Cvar_Set( "ui_Q3Model", "0" );
		}

		sys->Cvar_Set( "ui_gameType", va( "%d", ui_gameType.integer ) );
...
static qboolean UI_NetGameType_HandleKey( int flags, float *special, int key ) {
	...
	if ( key == K_MOUSE1 || key == K_MOUSE2 || key == K_ENTER || key == K_KP_ENTER ) {
		...
		sys->Cvar_Set( "ui_netGameType", va( "%d", ui_netGameType.integer ) );
		sys->Cvar_Set( "ui_actualnetGameType", va( "%d", uiInfo.gameTypes[ui_netGameType.integer].gtEnum ) );
		sys->Cvar_Set( "ui_currentNetMap", "0" );
...
static qboolean UI_JoinGameType_HandleKey( int flags, float *special, int key ) {
	...
	if ( key == K_MOUSE1 || key == K_MOUSE2 || key == K_ENTER || key == K_KP_ENTER ) {
		...
		sys->Cvar_Set( "ui_joinGameType", va( "%d", ui_joinGameType.integer ) );
...
static qboolean UI_Skill_HandleKey( int flags, float *special, int key ) {
	if ( key == K_MOUSE1 || key == K_MOUSE2 || key == K_ENTER || key == K_KP_ENTER ) {
		int i = sys->Cvar_VariableValue( "g_spSkill" );
...
static qboolean UI_TeamName_HandleKey( int flags, float *special, int key, qboolean blue ) {
	if ( key == K_MOUSE1 || key == K_MOUSE2 || key == K_ENTER || key == K_KP_ENTER ) {
		...
		sys->Cvar_Set( ( blue ) ? "ui_blueTeam" : "ui_redTeam", uiInfo.teamList[i].teamName );
...
static qboolean UI_TeamMember_HandleKey( int flags, float *special, int key, qboolean blue, int num ) {
	if ( key == K_MOUSE1 || key == K_MOUSE2 || key == K_ENTER || key == K_KP_ENTER ) {
		...
		int value = sys->Cvar_VariableValue( cvar );
...
static qboolean UI_NetSource_HandleKey( int flags, float *special, int key ) {
	...
	if ( key == K_MOUSE1 || key == K_MOUSE2 || key == K_ENTER || key == K_KP_ENTER ) {
		...
		sys->Cvar_Set( "ui_netSource", va( "%d", ui_netSource.integer ) );
		return qtrue;
	}
#endif  // #ifdef MISSIONPACK
	return qfalse;
...
static qboolean UI_Crosshair_HandleKey( int flags, float *special, int key ) {
	if ( key == K_MOUSE1 || key == K_MOUSE2 || key == K_ENTER || key == K_KP_ENTER ) {
		...
		sys->Cvar_Set( "cg_drawCrosshair", va( "%d", uiInfo.currentCrosshair ) );
		return qtrue;
	}
	return qfalse;
}
...
static qboolean UI_SelectedPlayer_HandleKey( int flags, float *special, int key ) {
	if ( key == K_MOUSE1 || key == K_MOUSE2 || key == K_ENTER || key == K_KP_ENTER ) {
		...
		selected = sys->Cvar_VariableValue( "cg_selectedPlayer" );
		...
		if ( selected == uiInfo.myTeamCount ) {
			sys->Cvar_Set( "cg_selectedPlayerName", "Everyone" );
		} else {
			sys->Cvar_Set( "cg_selectedPlayerName", uiInfo.teamNames[selected] );
		}
		sys->Cvar_Set( "cg_selectedPlayer", va( "%d", selected ) );
	}
	return qfalse;
}
...
static int QDECL UI_ServersQsortCompare( const void *arg1, const void *arg2 ) {
#ifdef MISSIONPACK
	return sys->LAN_CompareServers( ui_netSource.integer, uiInfo.serverStatus.sortKey, uiInfo.serverStatus.sortDir, *(int*)arg1, *(int*)arg2 );
#else
	return qfalse;
#endif  // #ifdef MISSIONPACK
}
...
void UI_SavegameSort( int column, qboolean force ) {
	...
	if ( uiInfo.savegameCount ) {
		...
		sys->Cvar_Set( "ui_savegame", "" );

	} else {
		sys->Cvar_Set( "ui_savegameName", "" );
		sys->Cvar_Set( "ui_savegameInfo", "(no savegames)" );
	}
...
static void UI_LoadMods() {
	...
	uiInfo.modCount = 0;
	numdirs = sys->FS_GetFileList( "$modlist", "", dirlist, sizeof( dirlist ) );
	dirptr  = dirlist;
	for ( i = 0; i < numdirs; i++ ) {
...
static void UI_LoadTeams() {
	...
	count = sys->FS_GetFileList( "", "team", teamList, 4096 );
...
static void UI_DelSavegame() {
	...
	ret = sys->FS_Delete( va( "save/%s.svg", uiInfo.savegameList[i].savegameFile ) );
...
void UI_ParseSavegame( int index ) {
	...
	sys->FS_FOpenFile( va( "save/%s.svg", uiInfo.savegameList[index].savegameFile ), &f, FS_READ );
	if ( !f ) {
		return;
	}

	// read the version
	sys->FS_Read( &ver, sizeof( i ), f );
	...
	// read the mapname
	sys->FS_Read( mapname, MAX_QPATH, f );
	...
	if ( ver > 14 ) {
		...
		sys->FS_Read( &tm->tm_sec, sizeof( tm->tm_sec ), f );          // secs after the min
		sys->FS_Read( &tm->tm_min, sizeof( tm->tm_min ), f );          // mins after the hour
		sys->FS_Read( &tm->tm_hour, sizeof( tm->tm_hour ), f );        // hrs since midnight
		sys->FS_Read( &tm->tm_mday, sizeof( tm->tm_mday ), f );
		sys->FS_Read( &tm->tm_mon, sizeof( tm->tm_mon ), f );
		sys->FS_Read( &tm->tm_year, sizeof( tm->tm_year ), f );        // yrs from 1900
		sys->FS_Read( &tm->tm_wday, sizeof( tm->tm_wday ), f );
		sys->FS_Read( &tm->tm_yday, sizeof( tm->tm_yday ), f );        // days since jan1 (0-365)
		sys->FS_Read( &tm->tm_isdst, sizeof( tm->tm_isdst ), f );
...
static void UI_LoadSavegames( char *dir ) {
	...
	if ( dir ) {
		uiInfo.savegameCount = sys->FS_GetFileList( va( "save/%s", dir ), "svg", sglist, 4096 );
	} else {
		uiInfo.savegameCount = sys->FS_GetFileList( "save", "svg", sglist, 4096 );
	}
...
//			qsort( &uiInfo.savegameStatus.displaySavegames[0], uiInfo.savegameCount, sizeof(int), UI_SavegamesQsortCompare);
			...
			if ( uiInfo.savegameList[i].episode != -1 ) {
				uiInfo.savegameList[i].sshotImage = sys->R_RegisterShaderNoMip( va( "levelshots/episodeshots/e%d.tga", uiInfo.savegameList[i].episode + 1 ) );
			} else {
				uiInfo.savegameList[i].sshotImage = sys->R_RegisterShaderNoMip( "levelshots/episodeshots/e_unknown.tga" );
			}
...
static void UI_LoadMovies() {
	...
	uiInfo.movieCount = sys->FS_GetFileList( "video", "roq", movielist, 4096 );
...
static void UI_LoadDemos() {
	...
	Com_sprintf( demoExt, sizeof( demoExt ), "dm_%d", (int)sys->Cvar_VariableValue( "protocol" ) );

	uiInfo.demoCount = sys->FS_GetFileList( "demos", demoExt, demolist, 4096 );
...
static void UI_StartSkirmish( qboolean next ) {
	...
	if ( next ) {
		int actual;
		int index = sys->Cvar_VariableValue( "ui_mapIndex" );
	...
	sys->Cvar_SetValue( "g_gametype", g );
	sys->Cmd_ExecuteText( EXEC_APPEND, va( "wait ; wait ; map %s\n", uiInfo.mapList[ui_currentMap.integer].mapLoadName ) );
	skill = sys->Cvar_VariableValue( "g_spSkill" );
	sys->Cvar_Set( "ui_scoreMap", uiInfo.mapList[ui_currentMap.integer].mapName );

	k = UI_TeamIndexFromName( UI_Cvar_VariableString( "ui_opponentName" ) );

	sys->Cvar_Set( "ui_singlePlayerActive", "1" );

	// set up sp overrides, will be replaced on postgame
	temp = sys->Cvar_VariableValue( "capturelimit" );
	sys->Cvar_Set( "ui_saveCaptureLimit", va( "%i", temp ) );
	temp = sys->Cvar_VariableValue( "fraglimit" );
	sys->Cvar_Set( "ui_saveFragLimit", va( "%i", temp ) );

	UI_SetCapFragLimits( qfalse );

	temp = sys->Cvar_VariableValue( "cg_drawTimer" );
	sys->Cvar_Set( "ui_drawTimer", va( "%i", temp ) );
	temp = sys->Cvar_VariableValue( "g_doWarmup" );
	sys->Cvar_Set( "ui_doWarmup", va( "%i", temp ) );
	temp = sys->Cvar_VariableValue( "g_friendlyFire" );
	sys->Cvar_Set( "ui_friendlyFire", va( "%i", temp ) );
	temp = sys->Cvar_VariableValue( "sv_maxClients" );
	sys->Cvar_Set( "ui_maxClients", va( "%i", temp ) );
	temp = sys->Cvar_VariableValue( "g_warmup" );
	sys->Cvar_Set( "ui_Warmup", va( "%i", temp ) );
	temp = sys->Cvar_VariableValue( "sv_pure" );
	sys->Cvar_Set( "ui_pure", va( "%i", temp ) );

	sys->Cvar_Set( "cg_cameraOrbit", "0" );
	sys->Cvar_Set( "cg_thirdPerson", "0" );
	sys->Cvar_Set( "cg_drawTimer", "1" );
	sys->Cvar_Set( "g_doWarmup", "1" );
	sys->Cvar_Set( "g_warmup", "15" );
	sys->Cvar_Set( "sv_pure", "0" );
	sys->Cvar_Set( "g_friendlyFire", "0" );
	sys->Cvar_Set( "g_redTeam", UI_Cvar_VariableString( "ui_teamName" ) );
	sys->Cvar_Set( "g_blueTeam", UI_Cvar_VariableString( "ui_opponentName" ) );

	if ( sys->Cvar_VariableValue( "ui_recordSPDemo" ) ) {
		Com_sprintf( buff, MAX_STRING_CHARS, "%s_%i", uiInfo.mapList[ui_currentMap.integer].mapLoadName, g );
		sys->Cvar_Set( "ui_recordSPDemoName", buff );
	}

	delay = 500;

	if ( g == GT_TOURNAMENT ) {
		sys->Cvar_Set( "sv_maxClients", "2" );
		Com_sprintf( buff, sizeof( buff ), "wait ; addbot %s %f " ", %i \n", uiInfo.mapList[ui_currentMap.integer].opponentName, skill, delay );
		sys->Cmd_ExecuteText( EXEC_APPEND, buff );
	} else {
		temp = uiInfo.mapList[ui_currentMap.integer].teamMembers * 2;
		sys->Cvar_Set( "sv_maxClients", va( "%d", temp ) );
	...
	if ( g >= GT_TEAM ) {
		sys->Cmd_ExecuteText( EXEC_APPEND, "wait 5; team Red\n" );
	}
#endif  // #ifdef MISSIONPACK
}
...
int WM_getWeaponIndex() {
	...
	lookupIndex = sys->Cvar_VariableValue( "mp_weapon" );
...
static void WM_ChangePlayerType() {
	...
	playerType = sys->Cvar_VariableValue( "mp_playerType" );
...
void WM_GetSpawnPoints() {
	...
	sys->GetConfigString( CS_MULTI_INFO, cs, sizeof( cs ) );
	...
	for ( i = 1; i < uiInfo.spawnCount; i++ ) {
		sys->GetConfigString( CS_MULTI_SPAWNTARGETS + i, cs, sizeof( cs ) );
...
void WM_PickItem( int selectionType, int itemIndex ) {
	...
	if ( selectionType == WM_SELECT_TEAM ) {
		switch ( itemIndex ) {
		case WM_AXIS:
			sys->Cvar_Set( "mp_team", "0" );
			break;
		case WM_ALLIES:
			sys->Cvar_Set( "mp_team", "1" );
			break;
		case WM_SPECTATOR:
			sys->Cvar_Set( "mp_team", "2" );
			break;
		}
		...
		case WM_START_SELECT:
			break;
		case WM_SOLDIER:
			sys->Cvar_Set( "mp_playerType", "0" );
			sys->Cvar_Set( "mp_weapon", "0" );
			break;
		case WM_MEDIC:
			sys->Cvar_Set( "mp_playerType", "1" );
			sys->Cvar_Set( "mp_weapon", "0" );
			break;
		case WM_ENGINEER:
			sys->Cvar_Set( "mp_playerType", "2" );
			sys->Cvar_Set( "mp_weapon", "0" );
			break;
		case WM_LIEUTENANT:
			sys->Cvar_Set( "mp_playerType", "3" );
			sys->Cvar_Set( "mp_weapon", "0" );
			break;
		}
		...
		if ( itemIndex == WM_START_SELECT ) {
			...
			sys->Cvar_Set( weaponTypes[itemIndex].cvar, va( "%i", weaponTypes[itemIndex].value ) );
		}
...
void WM_LimboChat() {
	...
	sys->Cvar_VariableStringBuffer( "ui_cmd", buf, 200 );

	if ( strlen( buf ) ) {
		sys->Cmd_ExecuteText( EXEC_APPEND, va( "say_limbo %s\n", buf ) );
	}
...
static void UI_Update( const char *name ) {
	int val = sys->Cvar_VariableValue( name );

	if ( Q_stricmp( name, "ui_SetName" ) == 0 ) {
		sys->Cvar_Set( "name", UI_Cvar_VariableString( "ui_Name" ) );
	} else if ( Q_stricmp( name, "ui_setRate" ) == 0 ) {
		float rate = sys->Cvar_VariableValue( "rate" );
		if ( rate >= 5000 ) {
			sys->Cvar_Set( "cl_maxpackets", "30" );
			sys->Cvar_Set( "cl_packetdup", "1" );
		} else if ( rate >= 4000 ) {
			sys->Cvar_Set( "cl_maxpackets", "15" );
			sys->Cvar_Set( "cl_packetdup", "2" );       // favor less prediction errors when there's packet loss
		} else {
			sys->Cvar_Set( "cl_maxpackets", "15" );
			sys->Cvar_Set( "cl_packetdup", "1" );       // favor lower bandwidth
		}
	} else if ( Q_stricmp( name, "ui_GetName" ) == 0 ) {
		sys->Cvar_Set( "ui_Name", UI_Cvar_VariableString( "name" ) );
	} else if ( Q_stricmp( name, "r_colorbits" ) == 0 ) {
		switch ( val ) {
		case 0:
			sys->Cvar_SetValue( "r_depthbits", 0 );
			sys->Cvar_SetValue( "r_stencilbits", 0 );
			break;
		case 16:
			sys->Cvar_SetValue( "r_depthbits", 16 );
			sys->Cvar_SetValue( "r_stencilbits", 0 );
			break;
		case 32:
			sys->Cvar_SetValue( "r_depthbits", 24 );
			break;
		}
	} else if ( Q_stricmp( name, "r_lodbias" ) == 0 ) {
		switch ( val ) {
		case 0:
			sys->Cvar_SetValue( "r_subdivisions", 4 );
			break;
		case 1:
			sys->Cvar_SetValue( "r_subdivisions", 12 );
			break;
		case 2:
			sys->Cvar_SetValue( "r_subdivisions", 20 );
			break;
		}
	} else if ( Q_stricmp( name, "ui_glCustom" ) == 0 ) {
		switch ( val ) {
		case 0: // high quality
			sys->Cvar_SetValue( "r_fullScreen", 1 );
			sys->Cvar_SetValue( "r_subdivisions", 4 );
			sys->Cvar_SetValue( "r_vertexlight", 0 );
			sys->Cvar_SetValue( "r_lodbias", 0 );
			sys->Cvar_SetValue( "r_colorbits", 32 );
			sys->Cvar_SetValue( "r_depthbits", 24 );
			sys->Cvar_SetValue( "r_picmip", 0 );
			sys->Cvar_SetValue( "r_picmip2", 0 );
			sys->Cvar_SetValue( "r_mode", 4 );
			sys->Cvar_SetValue( "r_texturebits", 32 );
			sys->Cvar_SetValue( "r_fastSky", 0 );
			sys->Cvar_SetValue( "r_inGameVideo", 1 );
			sys->Cvar_SetValue( "cg_shadows", 1 );
			sys->Cvar_SetValue( "cg_brassTime", 2500 );
			sys->Cvar_Set( "r_texturemode", "GL_LINEAR_MIPMAP_LINEAR" );
			break;
		case 1: // normal
			sys->Cvar_SetValue( "r_fullScreen", 1 );
			sys->Cvar_SetValue( "r_subdivisions", 12 );
			sys->Cvar_SetValue( "r_vertexlight", 0 );
			sys->Cvar_SetValue( "r_lodbias", 0 );
			sys->Cvar_SetValue( "r_colorbits", 0 );
			sys->Cvar_SetValue( "r_depthbits", 24 );
			sys->Cvar_SetValue( "r_picmip", 0 );
			sys->Cvar_SetValue( "r_picmip2", 1 );
			sys->Cvar_SetValue( "r_mode", 3 );
			sys->Cvar_SetValue( "r_texturebits", 0 );
			sys->Cvar_SetValue( "r_fastSky", 0 );
			sys->Cvar_SetValue( "r_inGameVideo", 1 );
			sys->Cvar_SetValue( "cg_brassTime", 2500 );
			sys->Cvar_Set( "r_texturemode", "GL_LINEAR_MIPMAP_NEAREST" ); //----(SA)	modified so wolf never sets trilinear automatically
			sys->Cvar_SetValue( "cg_shadows", 0 );
			break;
		case 2: // fast
			sys->Cvar_SetValue( "r_fullScreen", 1 );
			sys->Cvar_SetValue( "r_subdivisions", 8 );
			sys->Cvar_SetValue( "r_vertexlight", 0 );
			sys->Cvar_SetValue( "r_lodbias", 1 );
			sys->Cvar_SetValue( "r_colorbits", 0 );
			sys->Cvar_SetValue( "r_depthbits", 0 );
			sys->Cvar_SetValue( "r_picmip", 1 );
			sys->Cvar_SetValue( "r_picmip2", 2 );
			sys->Cvar_SetValue( "r_mode", 3 );
			sys->Cvar_SetValue( "r_texturebits", 0 );
			sys->Cvar_SetValue( "cg_shadows", 0 );
			sys->Cvar_SetValue( "r_fastSky", 1 );
			sys->Cvar_SetValue( "r_inGameVideo", 0 );
			sys->Cvar_SetValue( "cg_brassTime", 0 );
			sys->Cvar_Set( "r_texturemode", "GL_LINEAR_MIPMAP_NEAREST" );
			break;
		case 3: // fastest
			sys->Cvar_SetValue( "r_fullScreen", 1 );
			sys->Cvar_SetValue( "r_subdivisions", 20 );
			sys->Cvar_SetValue( "r_vertexlight", 1 );
			sys->Cvar_SetValue( "r_lodbias", 2 );
			sys->Cvar_SetValue( "r_colorbits", 16 );
			sys->Cvar_SetValue( "r_depthbits", 16 );
			sys->Cvar_SetValue( "r_mode", 3 );
			sys->Cvar_SetValue( "r_picmip", 2 );
			sys->Cvar_SetValue( "r_picmip2", 3 );
			sys->Cvar_SetValue( "r_texturebits", 16 );
			sys->Cvar_SetValue( "cg_shadows", 0 );
			sys->Cvar_SetValue( "cg_brassTime", 0 );
			sys->Cvar_SetValue( "r_fastSky", 1 );
			sys->Cvar_SetValue( "r_inGameVideo", 0 );
		...
		if ( val == 0 ) {
			sys->Cvar_SetValue( "m_pitch", 0.022f );
		} else {
			sys->Cvar_SetValue( "m_pitch", -0.022f );
		}
...
static void UI_RunMenuScript( char **args ) {
	...
	if ( String_Parse( args, &name ) ) {
		...
		if ( Q_stricmp( name, "StartServer" ) == 0 ) {
			...
			sys->Cvar_Set( "cg_thirdPerson", "0" );
			sys->Cvar_Set( "cg_cameraOrbit", "0" );
			sys->Cvar_Set( "ui_singlePlayerActive", "0" );
			sys->Cvar_SetValue( "dedicated", Com_Clamp( 0, 2, ui_dedicated.integer ) );
			sys->Cvar_SetValue( "g_gametype", Com_Clamp( 0, 8, uiInfo.gameTypes[ui_netGameType.integer].gtEnum ) );
			sys->Cmd_ExecuteText( EXEC_APPEND, va( "wait ; wait ; map %s\n", uiInfo.mapList[ui_currentNetMap.integer].mapLoadName ) );
			skill = sys->Cvar_VariableValue( "g_spSkill" );
			// set max clients based on spots
			clients = 0;
			for ( i = 0; i < PLAYERS_PER_TEAM; i++ ) {
				int bot = sys->Cvar_VariableValue( va( "ui_blueteam%i", i + 1 ) );
				if ( bot >= 0 ) {
					clients++;
				}
				bot = sys->Cvar_VariableValue( va( "ui_redteam%i", i + 1 ) );
				if ( bot >= 0 ) {
					clients++;
				}
			}
			...
			sys->Cvar_Set( "sv_maxClients", va( "%d",clients ) );
			...
			sys->Cvar_Set( "ui_mapIndex", va( "%d", ui_mapIndex.integer ) );
			Menu_SetFeederSelection( NULL, FEEDER_MAPS, ui_mapIndex.integer, "skirmish" );
			UI_GameType_HandleKey( 0, 0, K_MOUSE1, qfalse );
			UI_GameType_HandleKey( 0, 0, K_MOUSE2, qfalse );
		} else if ( Q_stricmp( name, "resetDefaults" ) == 0 ) {
// SP was like...
//			sys->Cmd_ExecuteText( EXEC_APPEND, "exec default.cfg\n");
//			sys->Cmd_ExecuteText( EXEC_APPEND, "cvar_restart\n");
//			Controls_SetDefaults();
//			sys->Cvar_Set("com_introPlayed", "1" );
//			sys->Cmd_ExecuteText( EXEC_APPEND, "vid_restart\n" );

// from MP 11/12/01
			sys->Cmd_ExecuteText( EXEC_NOW, "cvar_restart\n" );            // NERVE - SMF - changed order
			sys->Cmd_ExecuteText( EXEC_NOW, "exec default.cfg\n" );
			sys->Cmd_ExecuteText( EXEC_NOW, "exec language.cfg\n" );       // NERVE - SMF
			sys->Cmd_ExecuteText( EXEC_NOW, "setRecommended\n" );     // NERVE - SMF
			Controls_SetDefaults();
			sys->Cvar_Set( "com_introPlayed", "1" );
			sys->Cvar_Set( "com_recommendedSet", "1" );                   // NERVE - SMF
			sys->Cmd_ExecuteText( EXEC_APPEND, "vid_restart\n" );
// end from MP
		} else if ( Q_stricmp( name, "getCDKey" ) == 0 ) {
			char out[17];
			sys->GetCDKey( buff, 17 );
			sys->Cvar_Set( "cdkey1", "" );
			sys->Cvar_Set( "cdkey2", "" );
			sys->Cvar_Set( "cdkey3", "" );
			sys->Cvar_Set( "cdkey4", "" );
			if ( strlen( buff ) == CDKEY_LEN ) {
				...
				sys->Cvar_Set( "cdkey1", out );
				Q_strncpyz( out, buff + 4, 5 );
				sys->Cvar_Set( "cdkey2", out );
				Q_strncpyz( out, buff + 8, 5 );
				sys->Cvar_Set( "cdkey3", out );
				Q_strncpyz( out, buff + 12, 5 );
				sys->Cvar_Set( "cdkey4", out );
			}
			...
			sys->Cvar_Set( "cdkey", buff );
			if ( sys->VerifyCDKey( buff, UI_Cvar_VariableString( "cdkeychecksum" ) ) ) {
				sys->Cvar_Set( "ui_cdkeyvalid", "CD Key Appears to be valid." );
				sys->SetCDKey( buff );
			} else {
				sys->Cvar_Set( "ui_cdkeyvalid", "CD Key does not appear to be valid." );
			}
			//#ifdef MISSIONPACK			// NERVE - SMF - enabled for multiplayer
			...
			if ( uiInfo.demoAvailable ) {
				sys->Cmd_ExecuteText( EXEC_APPEND, va( "demo %s_%i", uiInfo.mapList[ui_currentMap.integer].mapLoadName, uiInfo.gameTypes[ui_gameType.integer].gtEnum ) );
			}
			//#endif	// #ifdef MISSIONPACK`
			...
			if ( DC->getCVarValue( "developer" ) ) {
				...
				sys->Cmd_ExecuteText( EXEC_APPEND, va( "loadgame %s\n", uiInfo.savegameList[i].savegameFile ) );
			}
			...
			sys->Cmd_ExecuteText( EXEC_APPEND, "fade 0 0 0 0 3\n" );    // fade screen up
			sys->Cvar_Set( "g_playerstart", "1" );                 // set cvar which will trigger "playerstart" in script
			Menus_CloseAll();
			//----(SA)	end
			...
			if ( uiInfo.previewMovie >= 0 ) {
				sys->CIN_StopCinematic( uiInfo.previewMovie );
			}
			sys->Cmd_ExecuteText( EXEC_APPEND, va( "cinematic %s.roq 2\n", uiInfo.movieList[uiInfo.movieIndex] ) );
		} else if ( Q_stricmp( name, "RunMod" ) == 0 ) {
			sys->Cvar_Set( "fs_game", uiInfo.modList[uiInfo.modIndex].modName );
			sys->Cmd_ExecuteText( EXEC_APPEND, "vid_restart;" );
		} else if ( Q_stricmp( name, "RunDemo" ) == 0 ) {
			sys->Cmd_ExecuteText( EXEC_APPEND, va( "demo %s\n", uiInfo.demoList[uiInfo.demoIndex] ) );
		} else if ( Q_stricmp( name, "Wolf" ) == 0 ) {
			sys->Cvar_Set( "fs_game", "" );
			sys->Cmd_ExecuteText( EXEC_APPEND, "vid_restart;" );
			//#ifdef MISSIONPACK			// NERVE - SMF - enabled for multiplayer
			...
			sys->LAN_GetServerAddressString( ui_netSource.integer, uiInfo.serverStatus.displayServers[uiInfo.serverStatus.currentServer], uiInfo.serverStatusAddress, sizeof( uiInfo.serverStatusAddress ) );
			...
			if ( uiInfo.serverStatus.currentServer >= 0 && uiInfo.serverStatus.currentServer < uiInfo.serverStatus.numDisplayServers ) {
				sys->LAN_GetServerAddressString( ui_netSource.integer, uiInfo.serverStatus.displayServers[uiInfo.serverStatus.currentServer], buff, 1024 );
				sys->Cmd_ExecuteText( EXEC_APPEND, va( "connect %s\n", buff ) );
			}
			...
			sys->Cvar_Set( "ui_singlePlayerActive", "0" );
			sys->Cmd_ExecuteText( EXEC_NOW, "quit" );
		} else if ( Q_stricmp( name, "Controls" ) == 0 ) {
			sys->Cvar_Set( "cl_paused", "1" );
			sys->Key_SetCatcher( KEYCATCH_UI );
			Menus_CloseAll();
			Menus_ActivateByName( "setup_menu2" );
		} else if ( Q_stricmp( name, "Leave" ) == 0 ) {
			sys->Cmd_ExecuteText( EXEC_APPEND, "disconnect\n" );
			sys->Key_SetCatcher( KEYCATCH_UI );
			Menus_CloseAll();
			...
			sys->Key_SetCatcher( sys->Key_GetCatcher() & ~KEYCATCH_UI );
			sys->Key_ClearStates();
			sys->Cvar_Set( "cl_paused", "0" );
			Menus_CloseAll();
			//#ifdef MISSIONPACK			// NERVE - SMF - enabled for multiplayer
		} else if ( Q_stricmp( name, "voteMap" ) == 0 ) {
			if ( ui_currentNetMap.integer >= 0 && ui_currentNetMap.integer < uiInfo.mapCount ) {
				sys->Cmd_ExecuteText( EXEC_APPEND, va( "callvote map %s\n",uiInfo.mapList[ui_currentNetMap.integer].mapLoadName ) );
			}
			//#endif	// #ifdef MISSIONPACK
		} else if ( Q_stricmp( name, "voteKick" ) == 0 ) {
			if ( uiInfo.playerIndex >= 0 && uiInfo.playerIndex < uiInfo.playerCount ) {
				sys->Cmd_ExecuteText( EXEC_APPEND, va( "callvote kick %s\n",uiInfo.playerNames[uiInfo.playerIndex] ) );
			}
		} else if ( Q_stricmp( name, "voteGame" ) == 0 ) {
			if ( ui_netGameType.integer >= 0 && ui_netGameType.integer < uiInfo.numGameTypes ) {
				sys->Cmd_ExecuteText( EXEC_APPEND, va( "callvote g_gametype %i\n",uiInfo.gameTypes[ui_netGameType.integer].gtEnum ) );
			}
		} else if ( Q_stricmp( name, "voteLeader" ) == 0 ) {
			if ( uiInfo.teamIndex >= 0 && uiInfo.teamIndex < uiInfo.myTeamCount ) {
				sys->Cmd_ExecuteText( EXEC_APPEND, va( "callteamvote leader %s\n",uiInfo.teamNames[uiInfo.teamIndex] ) );
			}
			//#ifdef MISSIONPACK			// NERVE - SMF - enabled for multiplayer
		} else if ( Q_stricmp( name, "addBot" ) == 0 ) {
			if ( sys->Cvar_VariableValue( "g_gametype" ) >= GT_TEAM ) {
				sys->Cmd_ExecuteText( EXEC_APPEND, va( "addbot %s %i %s\n", uiInfo.characterList[uiInfo.botIndex].name, uiInfo.skillIndex + 1, ( uiInfo.redBlue == 0 ) ? "Red" : "Blue" ) );
			} else {
				// NERVE - SMF - no bots in wolf multiplayer
			...
			if ( ui_netSource.integer != AS_FAVORITES ) {
				...
				sys->LAN_GetServerInfo( ui_netSource.integer, uiInfo.serverStatus.displayServers[uiInfo.serverStatus.currentServer], buff, MAX_STRING_CHARS );
				name[0] = addr[0] = '\0';
				Q_strncpyz( name,    Info_ValueForKey( buff, "hostname" ), MAX_NAME_LENGTH );
				Q_strncpyz( addr,    Info_ValueForKey( buff, "addr" ), MAX_NAME_LENGTH );
				if ( strlen( name ) > 0 && strlen( addr ) > 0 ) {
					res = sys->LAN_AddServer( AS_FAVORITES, name, addr );
					if ( res == 0 ) {
			...
			if ( ui_netSource.integer == AS_FAVORITES ) {
				...
				Q_strncpyz( addr,    Info_ValueForKey( buff, "addr" ), MAX_NAME_LENGTH );
				if ( strlen( addr ) > 0 ) {
					sys->LAN_RemoveServer( AS_FAVORITES, addr );
				}
			}
			...
			if ( String_Parse( args, &orders ) ) {
				int selectedPlayer = sys->Cvar_VariableValue( "cg_selectedPlayer" );
				if ( selectedPlayer < uiInfo.myTeamCount ) {
					strcpy( buff, orders );
					sys->Cmd_ExecuteText( EXEC_APPEND, va( buff, uiInfo.teamClientNums[selectedPlayer] ) );
					sys->Cmd_ExecuteText( EXEC_APPEND, "\n" );
				} else {
					int i;
					for ( i = 0; i < uiInfo.myTeamCount; i++ ) {
						...
						strcpy( buff, orders );
						sys->Cmd_ExecuteText( EXEC_APPEND, va( buff, uiInfo.teamNames[i] ) );
						sys->Cmd_ExecuteText( EXEC_APPEND, "\n" );
					}
				}
				sys->Key_SetCatcher( sys->Key_GetCatcher() & ~KEYCATCH_UI );
			...
			if ( String_Parse( args, &orders ) ) {
				int selectedPlayer = sys->Cvar_VariableValue( "cg_selectedPlayer" );
				if ( selectedPlayer == uiInfo.myTeamCount ) {
					sys->Cmd_ExecuteText( EXEC_APPEND, orders );
					sys->Cmd_ExecuteText( EXEC_APPEND, "\n" );
				}
				sys->Key_SetCatcher( sys->Key_GetCatcher() & ~KEYCATCH_UI );
				sys->Key_ClearStates();
...
//----(SA)	// start other .exe
			...
			sys->Cmd_ExecuteText( EXEC_APPEND, "startMultiplayer\n" );
...
//----(SA)
			...
			// get cvars
			team = sys->Cvar_VariableValue( "mp_team" );
			playerType = sys->Cvar_VariableValue( "mp_playerType" );
			weapon = sys->Cvar_VariableValue( "mp_weapon" );
			pistol = sys->Cvar_VariableValue( "mp_pistol" );
			item1 = sys->Cvar_VariableValue( "mp_item1" );
			...
			if ( team != 2 ) {
				sys->Cmd_ExecuteText( EXEC_APPEND, va( "limbomessage %s %s %s\n", teamStr, classStr, weapStr ) );
			}

			// join team
			if ( team == 0 ) {
				sys->Cmd_ExecuteText( EXEC_APPEND, va( "team %s %i %i %i %i 1\n", "red", playerType, weapon, pistol, item1 ) );
			} else if ( team == 1 ) {
			...
			sys->Cmd_ExecuteText( EXEC_APPEND, "setRecommended 1\n" );
		} else {
...
static void UI_BinaryServerInsertion( int num ) {
	...
	while ( mid > 0 ) {
		mid = len >> 1;
		//
		res = sys->LAN_CompareServers( ui_netSource.integer, uiInfo.serverStatus.sortKey,
...
static void UI_BuildServerDisplayList( qboolean force ) {
	...
	sys->Cvar_VariableStringBuffer( "cl_motdString", uiInfo.serverStatus.motd, sizeof( uiInfo.serverStatus.motd ) );
	...
	if ( force ) {
		...
		sys->LAN_MarkServerVisible( ui_netSource.integer, -1, qtrue );
	}

	// get the server count (comes from the master)
	count = sys->LAN_GetServerCount( ui_netSource.integer );
	...
	for ( i = 0; i < count; i++ ) {
		...
		if ( !sys->LAN_ServerIsVisible( ui_netSource.integer, i ) ) {
			continue;
		}
		visible = qtrue;
		// get the ping for this server
		ping = sys->LAN_GetServerPing( ui_netSource.integer, i );
		if ( ping > 0 || ui_netSource.integer == AS_FAVORITES ) {

			sys->LAN_GetServerInfo( ui_netSource.integer, i, info, MAX_STRING_CHARS );
			...
			if ( ui_browserShowEmpty.integer == 0 ) {
				if ( clients == 0 ) {
					sys->LAN_MarkServerVisible( ui_netSource.integer, i, qfalse );
					continue;
				}
			}
...
static int UI_GetServerStatusInfo( const char *serverAddress, serverStatusInfo_t *info ) {
	...
	if ( !info ) {
		sys->LAN_ServerStatus( serverAddress, NULL, 0 );
		return qfalse;
	}
	memset( info, 0, sizeof( *info ) );
	if ( sys->LAN_ServerStatus( serverAddress, info->text, sizeof( info->text ) ) ) {
...
static void UI_BuildFindPlayerList( qboolean force ) {
	...
	if ( !force ) {
		...
		sys->Cvar_VariableStringBuffer( "ui_findPlayer", uiInfo.findPlayerName, sizeof( uiInfo.findPlayerName ) );
		...
		sys->Cvar_Set( "cl_serverStatusResendTime", va( "%d", resend ) );
		// reset all server status requests
		sys->LAN_ServerStatus( NULL, NULL, 0 );
		//
	...
	for ( i = 0; i < MAX_SERVERSTATUSREQUESTS; i++ ) {
		...
		if ( !uiInfo.pendingServerStatus.server[i].valid ||
			...
			if ( uiInfo.pendingServerStatus.num < uiInfo.serverStatus.numDisplayServers ) {
				...
				sys->LAN_GetServerAddressString( ui_netSource.integer, uiInfo.serverStatus.displayServers[uiInfo.pendingServerStatus.num],
												 uiInfo.pendingServerStatus.server[i].adrstr, sizeof( uiInfo.pendingServerStatus.server[i].adrstr ) );
				sys->LAN_GetServerInfo( ui_netSource.integer, uiInfo.serverStatus.displayServers[uiInfo.pendingServerStatus.num], infoString, sizeof( infoString ) );
...
static void UI_UpdatePendingPings() {
	//#ifdef MISSIONPACK			// NERVE - SMF - enabled for multiplayer
	sys->LAN_ResetPings( ui_netSource.integer );
	...
	if ( feederID == FEEDER_HEADS ) {
		...
		if ( index >= 0 && index < uiInfo.serverStatus.numDisplayServers ) {
			...
			if ( lastServerColumn != column || lastServerTime > uiInfo.uiDC.realTime + 5000 ) {
				sys->LAN_GetServerInfo( ui_netSource.integer, uiInfo.serverStatus.displayServers[index], info, MAX_STRING_CHARS );
	...
	if ( feederID == FEEDER_HEADS ) {
		if ( index >= 0 && index < uiInfo.characterCount ) {
			if ( uiInfo.characterList[index].headImage == -1 ) {
				uiInfo.characterList[index].headImage = sys->R_RegisterShaderNoMip( uiInfo.characterList[index].imageName );
			}
		...
		if ( index >= 0 && index < uiInfo.mapCount ) {
			if ( uiInfo.mapList[index].levelShot == -1 ) {
				uiInfo.mapList[index].levelShot = sys->R_RegisterShaderNoMip( uiInfo.mapList[index].imageName );
			}
		...
		if ( index >= 0 && index < uiInfo.savegameCount ) {
			if ( uiInfo.savegameList[uiInfo.savegameStatus.displaySavegames[index]].sshotImage == -1 ) {
				uiInfo.savegameList[uiInfo.savegameStatus.displaySavegames[index]].sshotImage = sys->R_RegisterShaderNoMip( va( "levelshots/%s.tga",uiInfo.savegameList[uiInfo.savegameStatus.displaySavegames[index]].mapName ) );
			}
...
static void UI_FeederSelection( float feederID, int index ) {
	...
	if ( feederID == FEEDER_HEADS ) {
		if ( index >= 0 && index < uiInfo.characterCount ) {
			sys->Cvar_Set( "team_model", uiInfo.characterList[index].female ? "janet" : "james" );
			sys->Cvar_Set( "team_headmodel", va( "*%s", uiInfo.characterList[index].name ) );
			updateModel = qtrue;
		}
	} else if ( feederID == FEEDER_Q3HEADS ) {
		if ( index >= 0 && index < uiInfo.q3HeadCount ) {
			sys->Cvar_Set( "model", uiInfo.q3HeadNames[index] );
			sys->Cvar_Set( "headmodel", uiInfo.q3HeadNames[index] );
			updateModel = qtrue;
		}
		//#ifdef MISSIONPACK			// NERVE - SMF - enabled for multiplayer
	} else if ( feederID == FEEDER_MAPS || feederID == FEEDER_ALLMAPS ) {
		...
		if ( uiInfo.mapList[map].cinematic >= 0 ) {
			sys->CIN_StopCinematic( uiInfo.mapList[map].cinematic );
			uiInfo.mapList[map].cinematic = -1;
		}
		UI_SelectedMap( index, &actual );
		sys->Cvar_Set( "ui_mapIndex", va( "%d", index ) );
		ui_mapIndex.integer = index;

		if ( feederID == FEEDER_MAPS ) {
			ui_currentMap.integer = actual;
			sys->Cvar_Set( "ui_currentMap", va( "%d", actual ) );
			uiInfo.mapList[ui_currentMap.integer].cinematic = sys->CIN_PlayCinematic( va( "%s.roq", uiInfo.mapList[ui_currentMap.integer].mapLoadName ), 0, 0, 0, 0, ( CIN_loop | CIN_silent ) );
			UI_LoadBestScores( uiInfo.mapList[ui_currentMap.integer].mapLoadName, uiInfo.gameTypes[ui_gameType.integer].gtEnum );
			sys->Cvar_Set( "ui_opponentModel", uiInfo.mapList[ui_currentMap.integer].opponentName );
			updateOpponentModel = qtrue;
		} else {
			ui_currentNetMap.integer = actual;
			sys->Cvar_Set( "ui_currentNetMap", va( "%d", actual ) );
			uiInfo.mapList[ui_currentNetMap.integer].cinematic = sys->CIN_PlayCinematic( va( "%s.roq", uiInfo.mapList[ui_currentNetMap.integer].mapLoadName ), 0, 0, 0, 0, ( CIN_loop | CIN_silent ) );
		}
		...
		sys->LAN_GetServerInfo( ui_netSource.integer, uiInfo.serverStatus.displayServers[index], info, MAX_STRING_CHARS );
		uiInfo.serverStatus.currentServerPreview = sys->R_RegisterShaderNoMip( va( "levelshots/%s", Info_ValueForKey( info, "mapname" ) ) );
		if ( uiInfo.serverStatus.currentServerCinematic >= 0 ) {
			sys->CIN_StopCinematic( uiInfo.serverStatus.currentServerCinematic );
			uiInfo.serverStatus.currentServerCinematic = -1;
		}
		mapName = Info_ValueForKey( info, "mapname" );
		if ( mapName && *mapName ) {
			uiInfo.serverStatus.currentServerCinematic = sys->CIN_PlayCinematic( va( "%s.roq", mapName ), 0, 0, 0, 0, ( CIN_loop | CIN_silent ) );
		}
	} else if ( feederID == FEEDER_SERVERSTATUS ) {
		//
	} else if ( feederID == FEEDER_FINDPLAYER ) {
...
//		itemDef_t *itemdef;
		...
		if ( index >= 0 && index < uiInfo.savegameCount ) {
			...
			// the text entry box
			sys->Cvar_Set( "ui_savegame", uiInfo.savegameList[uiInfo.savegameStatus.displaySavegames[index]].savegameName );

			sys->Cvar_Set( "ui_savegameName", uiInfo.savegameList[uiInfo.savegameStatus.displaySavegames[index]].savegameName );
			sys->Cvar_Set( "ui_savegameInfo", uiInfo.savegameList[uiInfo.savegameStatus.displaySavegames[index]].savegameInfoText );
...
//			itemdef->cursorPos = 0;
		...
		sys->Cmd_ExecuteText( EXEC_NOW, va( "setspawnpt %i\n", index ) );
	}
	// -NERVE - SMF
}
...
static qboolean Team_Parse(char **p) {
	...
	while ( 1 ) {
		...
		if (token[0] == '{') {
			...
			uiInfo.teamList[uiInfo.teamCount].teamIcon = sys->R_RegisterShaderNoMip(uiInfo.teamList[uiInfo.teamCount].imageName);
			uiInfo.teamList[uiInfo.teamCount].teamIcon_Metal = sys->R_RegisterShaderNoMip(va("%s_metal",uiInfo.teamList[uiInfo.teamCount].imageName));
			uiInfo.teamList[uiInfo.teamCount].teamIcon_Name = sys->R_RegisterShaderNoMip(va("%s_name", uiInfo.teamList[uiInfo.teamCount].imageName));
...
static qboolean MapList_Parse( char **p ) {
	...
	while ( 1 ) {
		...
		if ( token[0] == '{' ) {
			...
			//  uiInfo.mapList[uiInfo.mapCount].cinematic = sys->CIN_PlayCinematic(va("%s.roq",uiInfo.mapList[uiInfo.mapCount].mapLoadName), qfalse, qfalse, qtrue, 0, 0, 0, 0);
			//}
			uiInfo.mapList[uiInfo.mapCount].cinematic = -1;
			uiInfo.mapList[uiInfo.mapCount].levelShot = sys->R_RegisterShaderNoMip( va( "levelshots/%s_small", uiInfo.mapList[uiInfo.mapCount].mapLoadName ) );
...
static int UI_PlayCinematic( const char *name, float x, float y, float w, float h ) {
	return sys->CIN_PlayCinematic( name, x, y, w, h, ( CIN_loop | CIN_silent ) );
}
	...
	if ( handle >= 0 ) {
		...
		if ( handle == UI_MAPCINEMATIC ) {
			if ( uiInfo.mapList[ui_currentMap.integer].cinematic >= 0 ) {
				sys->CIN_StopCinematic( uiInfo.mapList[ui_currentMap.integer].cinematic );
...
static void UI_BuildQ3Model_List( void ) {
	...
	numdirs = sys->FS_GetFileList( "models/players", "/", dirlist, 2048 );
	dirptr  = dirlist;
	for ( i = 0; i < numdirs && uiInfo.q3HeadCount < MAX_PLAYERMODELS; i++,dirptr += dirlen + 1 )
	{
		...
		numfiles = sys->FS_GetFileList( va( "models/players/%s",dirptr ), "tga", filelist, 2048 );
		fileptr  = filelist;
		for ( j = 0; j < numfiles && uiInfo.q3HeadCount < MAX_PLAYERMODELS; j++,fileptr += filelen + 1 )
		{
			...
			if ( Q_stricmpn( skinname, "icon_", 5 ) == 0 && !( Q_stricmp( skinname,"icon_blue" ) == 0 || Q_stricmp( skinname,"icon_red" ) == 0 ) ) {
				...
				uiInfo.q3HeadIcons[uiInfo.q3HeadCount++] = sys->R_RegisterShaderNoMip( va( "models/players/%s/%s",dirptr,skinname ) );
			}
...
void _UI_Init( qboolean inGameLoad ) {
	...
	sys->GetGlconfig( &uiInfo.uiDC.glconfig );
	...
	uiInfo.uiDC.registerShaderNoMip = [](const char* p) -> qhandle_t {
		return sys->R_RegisterShaderNoMip(p);
		};
	uiInfo.uiDC.drawStretchPic = [](float x, float y, float w, float h,
		float s1, float t1, float s2, float t2,
		qhandle_t hShader) {
			sys->R_DrawStretchPic(x, y, w, h, s1, t1, s2, t2, hShader);
		};
	uiInfo.uiDC.registerModel = [](const char* p) -> qhandle_t {
		return sys->R_RegisterModel(p);
		};
	uiInfo.uiDC.modelBounds = [](qhandle_t model, vec3_t min, vec3_t max) {
		sys->R_ModelBounds(model, min, max);
		};
	uiInfo.uiDC.clearScene = []() {
		sys->R_ClearScene();
		};
	uiInfo.uiDC.addRefEntityToScene = [](const refEntity_t* re) {
		sys->R_AddRefEntityToScene(re);
		};
	uiInfo.uiDC.renderScene = [](const refdef_t* fd) {
		sys->R_RenderScene(fd);
		};
	uiInfo.uiDC.registerFont = [](const char* pFontname, int pointSize, fontInfo_t* font) {
		sys->R_RegisterFont(pFontname, pointSize, font);
		};

	uiInfo.uiDC.setCVar = [](const char* cvar, const char* value) {
		sys->Cvar_Set(cvar, value);
		};
	uiInfo.uiDC.getCVarString = [](const char* cvar, char* buffer, int bufsize) {
		sys->Cvar_VariableStringBuffer(cvar, buffer, bufsize);
		};
	uiInfo.uiDC.getCVarValue = [](const char* cvar) -> float {
		return sys->Cvar_VariableValue(cvar);
		};

	uiInfo.uiDC.setOverstrikeMode = [](qboolean b) {
		sys->Key_SetOverstrikeMode(b);
		};
	uiInfo.uiDC.getOverstrikeMode = []() -> qboolean {
		return sys->Key_GetOverstrikeMode();
		};
	uiInfo.uiDC.startLocalSound = [](sfxHandle_t sfx, int channelNum) {
		sys->S_StartLocalSound(sfx, channelNum);
		};

	uiInfo.uiDC.setBinding = [](int keynum, const char* binding) {
		sys->Key_SetBinding(keynum, binding);
		};
	uiInfo.uiDC.getBindingBuf = [](int keynum, char* buf, int buflen) {
		sys->Key_GetBindingBuf(keynum, buf, buflen);
		};
	uiInfo.uiDC.keynumToStringBuf = [](int keynum, char* buf, int buflen) {
		sys->Key_KeynumToStringBuf(keynum, buf, buflen);
		};
	uiInfo.uiDC.executeText = [](int exec_when, const char* text) {
		sys->Cmd_ExecuteText(exec_when, text);
		};

	uiInfo.uiDC.registerSound = [](const char* name) -> sfxHandle_t {
		return sys->S_RegisterSound(name);
		};
	uiInfo.uiDC.startBackgroundTrack = [](const char* intro, const char* loop, int fadeupTime) {
		sys->S_StartBackgroundTrack(intro, loop, fadeupTime);
		};
	uiInfo.uiDC.stopBackgroundTrack = []() {
...
//	uiInfo.uiDC.cursor	= sys->R_RegisterShaderNoMip( "menu/art/3_cursor3" );
	uiInfo.uiDC.whiteShader = sys->R_RegisterShaderNoMip( "white" );
...
//#endif	// #ifdef MISSIONPACK
	...
	uiInfo.effectsColor = gamecodetoui[(int)sys->Cvar_VariableValue( "color" ) - 1];
	uiInfo.currentCrosshair = (int)sys->Cvar_VariableValue( "cg_drawCrosshair" );
	sys->Cvar_Set( "ui_mousePitch", ( sys->Cvar_VariableValue( "m_pitch" ) >= 0 ) ? "0" : "1" );

	uiInfo.serverStatus.currentServerCinematic = -1;
	uiInfo.previewMovie = -1;

	if ( sys->Cvar_VariableValue( "ui_WolfFirstRun" ) == 0 ) {
		sys->Cvar_Set( "s_volume", "0.8" );
		sys->Cvar_Set( "s_musicvolume", "0.8" );
		sys->Cvar_Set( "ui_WolfFirstRun", "1" );
	}

	sys->Cvar_Register( NULL, "debug_protocol", "", 0 );
}
...
void _UI_KeyEvent( int key, qboolean down ) {
	...
	//  sys->S_StartLocalSound( s, CHAN_LOCAL_SOUND );
	//}
}
...
void _UI_SetActiveMenu( uiMenuCommand_t menu ) {
	...
	if ( Menu_Count() > 0 ) {
		...
		case UIMENU_MAIN:
			//sys->Cvar_Set( "sv_killserver", "1" );
			sys->Key_SetCatcher( KEYCATCH_UI );
			//sys->S_StartLocalSound( sys->S_RegisterSound("sound/misc/menu_background.wav", qfalse) , CHAN_LOCAL_SOUND );
			//sys->S_StartBackgroundTrack("sound/misc/menu_background.wav", NULL);
			if ( uiInfo.inGameLoad ) {
				UI_LoadNonIngame();
			}
			Menus_CloseAll();
			Menus_ActivateByName( "main" );
			sys->Cvar_VariableStringBuffer( "com_errorMessage", buf, sizeof( buf ) );
			if ( strlen( buf ) ) {
#ifdef MISSIONPACK
			...
			sys->S_FadeAllSound( 1.0f, 1000 );    // make sure sound fades up
...
static void UI_DisplayDownloadInfo( const char *downloadName, float centerPoint, float yStart, int font, float scale ) {
	...
	downloadSize = sys->Cvar_VariableValue( "cl_downloadSize" );
	downloadCount = sys->Cvar_VariableValue( "cl_downloadCount" );
	downloadTime = sys->Cvar_VariableValue( "cl_downloadTime" );
...
void UI_DrawConnectScreen( qboolean overlay ) {
	...
	sys->GetClientState( &cstate );

	info[0] = '\0';
	if ( sys->GetConfigString( CS_SERVERINFO, info, sizeof( info ) ) ) {
...
//		Text_PaintCenter(centerPoint, yStart + 48, UI_FONT_DEFAULT, scale, colorWhite, va("Get Psyched!"), ITEM_TEXTSTYLE_SHADOWEDMORE);
	...
	case CA_CONNECTED: {
		...
		sys->Cvar_VariableStringBuffer( "cl_downloadName", downloadName, sizeof( downloadName ) );
		if ( *downloadName ) {
...
static void UI_StopServerRefresh( void ) {
	...
	if ( count - uiInfo.serverStatus.numDisplayServers > 0 ) {
		Com_Printf( "%d servers not listed due to packet loss or pings higher than %d\n",
					...
					(int) sys->Cvar_VariableValue( "cl_maxPing" ) );
	}
...
static void UI_DoServerRefresh( void ) {
	...
	if ( ui_netSource.integer != AS_FAVORITES ) {
		if ( ui_netSource.integer == AS_LOCAL ) {
			if ( !sys->LAN_GetServerCount( ui_netSource.integer ) ) {
				wait = qtrue;
			}
		} else {
			if ( sys->LAN_GetServerCount( ui_netSource.integer ) < 0 ) {
				wait = qtrue;
			}
		}
	}
	...
	if ( sys->LAN_UpdateVisiblePings( ui_netSource.integer ) ) {
		uiInfo.serverStatus.refreshtime = uiInfo.uiDC.realTime + 1000;
	} else if ( !wait ) {
...
static void UI_StartServerRefresh( qboolean full ) {
	...
	qtime_t q;
	sys->RealTime( &q );
	sys->Cvar_Set( va( "ui_lastServerRefresh_%i", ui_netSource.integer ), va( "%s-%i, %i at %i:%i", MonthAbbrev[q.tm_mon],q.tm_mday, 1900 + q.tm_year,q.tm_hour,q.tm_min ) );
	...
	//
	if ( ui_netSource.integer == AS_LOCAL ) {
		sys->Cmd_ExecuteText( EXEC_NOW, "localservers\n" );
		uiInfo.serverStatus.refreshtime = uiInfo.uiDC.realTime + 1000;
		return;
	}
	...
	if ( ui_netSource.integer == AS_GLOBAL || ui_netSource.integer == AS_MPLAYER ) {
		...
		if ( strlen( ptr ) ) {
			sys->Cmd_ExecuteText( EXEC_NOW, va( "globalservers %d %s full empty\n", i, ptr ) );
		} else {
			sys->Cmd_ExecuteText( EXEC_NOW, va( "globalservers %d %d full empty\n", i, (int)sys->Cvar_VariableValue( "protocol" ) ) );
		}
	}
}
// -NERVE - SMF
