...
static void NeedCDAction( qboolean result ) {
	if ( !result ) {
		sys->Cmd_ExecuteText( EXEC_APPEND, "quit\n" );
	}
}
...
static void UI_CalcPostGameStats() {
	...
	sys->Cvar_Set( "capturelimit", UI_Cvar_VariableString( "ui_saveCaptureLimit" ) );
	sys->Cvar_Set( "fraglimit", UI_Cvar_VariableString( "ui_saveFragLimit" ) );
	sys->Cvar_Set( "cg_drawTimer", UI_Cvar_VariableString( "ui_drawTimer" ) );
	sys->Cvar_Set( "g_doWarmup", UI_Cvar_VariableString( "ui_doWarmup" ) );
	sys->Cvar_Set( "g_Warmup", UI_Cvar_VariableString( "ui_Warmup" ) );
	sys->Cvar_Set( "sv_pure", UI_Cvar_VariableString( "ui_pure" ) );
	sys->Cvar_Set( "g_friendlyFire", UI_Cvar_VariableString( "ui_friendlyFire" ) );
...
qboolean UI_ConsoleCommand( int realTime ) {
	...
	if ( Q_stricmp( cmd, "remapShader" ) == 0 ) {
		if ( sys->Argc() == 4 ) {
			...
			sys->R_RemapShader( shader1, shader2, UI_Argv( 3 ) );
			return qtrue;
		}
	}
...
void UI_DrawHandlePic( float x, float y, float w, float h, qhandle_t hShader ) {
	...
	sys->R_DrawStretchPic( x, y, w, h, s0, t0, s1, t1, hShader );
}
...
void UI_FillRect( float x, float y, float width, float height, const float *color ) {
	...
	UI_AdjustFrom640( &x, &y, &width, &height );
	sys->R_DrawStretchPic( x, y, width, height, 0, 0, 0, 0, uiInfo.uiDC.whiteShader );
