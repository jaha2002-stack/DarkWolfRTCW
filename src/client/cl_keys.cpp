...
//----(SA)	get the active menu if in ui mode
	if ( cls.keyCatchers & KEYCATCH_UI ) {
		activeMenu = uivm->GetActiveMenu();// VM_Call(uivm, UI_GET_ACTIVE_MENU);
	}
	...
	if ( key == K_ESCAPE && down ) {
		...
		if ( cls.keyCatchers & KEYCATCH_CGAME ) {
			...
			cgvm->EventHandling(CGAME_EVENT_NONE );
			return;
		}

		if ( !( cls.keyCatchers & KEYCATCH_UI ) ) {
			if ( cls.state == CA_ACTIVE && !clc.demoplaying ) {
				uivm->SetActiveMenu( UIMENU_INGAME );
			} else {
				CL_Disconnect_f();
				S_StopAllSounds();
		...
		if ( activeMenu == UIMENU_CLIPBOARD ) {
			...
			if ( kb ) {
				if ( !Q_stricmp( "notebook", kb ) ) {
					if ( uivm->GetActiveMenu() == UIMENU_NOTEBOOK ) {
						key = K_ESCAPE;
					}
				}
...
void CL_CharEvent( int key ) {
	...
	if ( cls.keyCatchers & KEYCATCH_CONSOLE ) {
		...
		uivm->KeyEvent( key | K_CHAR_FLAG, qtrue );
	} else if ( cls.keyCatchers & KEYCATCH_MESSAGE )   {
